...
已有 Node.js 22.20+ 的环境也可使用 npm：

```sh
npx -y @hiq-ai/cortex-org-wiki-cli@latest skill setup --agent codex
```

之后可继续以 `npx -y @hiq-ai/cortex-org-wiki-cli@latest` 代替命令名。Cortex Cowork 的市场安装由 Host 供给同源 skill 与 CLI；以该会话的实际 CLI 和身份检查结果确认是否接通。
